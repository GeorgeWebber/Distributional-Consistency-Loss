...
import parallelproj
from ..projector_utils import operators
...
class RegularPolygonPETScannerGeometry(ModularizedPETScannerGeometry):
    ...
    def __init__(self, xp: ModuleType, dev: str, radius: float, num_sides: int,
        ...
        for ring in range(num_rings):
            ...
            # hacky code that may not be general to symmetric axis. Adjusts the reconstruction by 3 degrees TODO - 
            rad = (-90+3) /180 * np.pi
            aff_mat[self._ax0,self._ax0] = np.cos(rad)
            aff_mat[self._ax1,self._ax0] = np.sin(rad)
            aff_mat[self._ax0,self._ax1] = -np.sin(rad)
            aff_mat[self._ax1,self._ax1] = np.cos(rad)
            aff_mat[symmetry_axis, -1] = ring_positions[ring]
            aff_mat[self._ax0, 3] = +0.65
            aff_mat[self._ax1, 3] = +0.65
...
class RegularPolygonPETLORDescriptor(PETLORDescriptor):
    ...
    def __init__(
        ...
        self._num_rad = (self.scanner.num_lor_endpoints_per_ring) - 2 * self._radial_trim
    ...
    def _setup_view_indices(self) -> None:
        ...
        for view in np.arange(self._num_views):
            self._start_in_ring_index[view, :] = (
                ...
                view)[self._radial_trim:-(self._radial_trim+1)]
            self._end_in_ring_index[view, :] = (
                self.xp.concat(
                    (self.xp.asarray([-1]), -((self.xp.arange(m) + 2) // 2))) -
                view)[self._radial_trim:-(self._radial_trim+1)]  
...
class RegularPolygonPETNonTOFProjector(operators.LinearOperator):
    ...
    def __init__(self,
        ...
        if img_origin is None:
            ...
            self._img_origin[2] = -self._voxel_size[2] / 2.0
        else:
        ...
        if resolution_model is None:
            self._resolution_model = operators.GaussianFilterOperator(
                self.in_shape, sigma= 4.5 / (2.355 * self._voxel_size))
        elif resolution_model == "identity":
            self._resolution_model = operators.IdentityOperator(
                self.in_shape)
        else:
    ...
    def show_geometry(
        ...
        x *= int(self._img_origin[0]) * float(self._voxel_size[0])
        y *= int(self._img_origin[1]) * float(self._voxel_size[1])
        z *= int(self._img_origin[2]) * float(self._voxel_size[2])

        x -= float(self._img_origin[0]) - 0.5 * float(self._voxel_size[0])
        y -= float(self._img_origin[1]) - 0.5 * float(self._voxel_size[1])
        z -= float(self._img_origin[2]) - 0.5 * float(self._voxel_size[2])
        ...
        self._lor_descriptor.scanner.show_lor_endpoints(ax)
